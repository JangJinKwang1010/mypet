...
<jsp:include page="../header.jsp"></jsp:include>
		...
		<div class="mainbox">
			...
			<div class ="mainbox2 freebox">
				...
				<form id="form">
				<select class="pet_select form-select" style="width:90%" name="kind">
					<c:if test="${vo.kind eq '강아지' }">
						<option value="${vo.kind }">${vo.kind }</option>
						<option value="고양이">고양이</option>
					</c:if>
					<c:if test="${vo.kind eq '고양이' }">
						<option value="${vo.kind }">${vo.kind }</option>
						<option value="강아지">강아지</option>
					</c:if>
				</select>
				<select class="pet_select form-select" style="width:90%" name="work">
					<c:if test="${vo.work eq '1년'}">
						<option value="1년" selected>1년</option>
						<option value="2년">2년<option>
						<option value="3년">3년</option>
						<option value="4년">4년</option>
						<option value="5년 이상">5년 이상</option>
					</c:if>
					<c:if test="${vo.work eq '2년'}">
					...
					<c:if test="${vo.work eq '3년'}">
						<option value="1년" >1년</option>
						<option value="2년" >2년<option>
						<option value="3년" selected>3년</option>
						<option value="4년">4년</option>
						<option value="5년 이상">5년 이상</option>
					</c:if>
					<c:if test="${vo.work eq '4년'}">
						<option value="1년" >1년</option>
						<option value="2년" >2년<option>
						<option value="3년" >3년</option>
						<option value="4년" selected>4년</option>
						<option value="5년 이상">5년 이상</option>
					</c:if>
				...
				<input type="hidden" name="nid2" value="${vo.nid2 }">
				</form>
			</div>
		</div>
	</section>
